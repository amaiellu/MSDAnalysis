...
def velocityCalcs(mydir):
        ...
        for file in files:
            ...
            finavg_per_frames.iloc[count]=[num_good,avg_particles_frame,fraction_moving]
        ...
        #write masterfile
        writer = pd.ExcelWriter(os.path.join(mydir,'Compiled Data Drift Velocities 09262016.xlsx'), engine='xlsxwriter')
        finframes_vs.to_excel(writer,sheet_name='Ind Particle Velocities')
        finvelocities.to_excel(writer,sheet_name='Frame by Frame Velocities')
        finavg_per_frames.to_excel(writer, sheet_name='Summary')
        writer.save()
...
velocityCalcs('C:\\Users\\amschaef\\Documents\\Bacteria Output\\test')
